...
function script__get_filename(string $experiment_id, int $visualization_index):string {
	$script_path = DATA_PUBLIC_DIR . '/scripts/';
	// xp_id = {time_stamp}_{config_id}_{viz_index}.py
	// the time stamp has to be removed
	// the TS is required to be disinguished several experiences from a same config
...
function script__exec(string $script_filename, mixed $data):mixed {
